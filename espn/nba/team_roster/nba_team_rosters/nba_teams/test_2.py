...
if not os.path.exists(team_folder):
    print(f"Error: {team_name} folder does not exist.")
    exit()

# Locate the team's roster CSV
roster_csv = None
...
if not roster_csv:
    print(f"No roster CSV found in {team_folder}, exiting...")
    exit()

def generate_player_script(player_id, player_name):
...
player_folder = os.path.join("nba_teams", "{team_name}", "{safe_player_name}")
...
# Loop through each line, print the details, and create player scripts
for line in lines:
    ...
    # Print the current row with player details
    print(f"Processing player: ID = {player_id}, Name = {player_name}")

    # Generate the player script
    generate_player_script(player_id, player_name)