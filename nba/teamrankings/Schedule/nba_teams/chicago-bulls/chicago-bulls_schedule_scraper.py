...
BASE_DIR = "/Users/kamahl/Sports/scripts/nba/teamrankings/Schedule/nba_teams/chicago-bulls"
os.makedirs(BASE_DIR, exist_ok=True)

def fetch_and_save_chicago_bulls_schedule():
    """Fetch and save the Chicago Bulls schedule from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/chicago-bulls"
    
    print(f"Fetching schedule for Chicago Bulls from {url}")
    ...
    # Save to CSV
    csv_filename = os.path.join(BASE_DIR, "chicago-bulls_schedule_results.csv")
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, "chicago-bulls_schedule_results.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table to terminal
    print(f"\nSchedule for Chicago Bulls")
...
if __name__ == "__main__":
    fetch_and_save_chicago_bulls_schedule()