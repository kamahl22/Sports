...
BASE_DIR = "/Users/kamahl/Sports/scripts/nba/teamrankings/players/roster/nba_teams"
...
# Template for team roster script with terminal printing
ROSTER_TEMPLATE = '''import pandas as pd
import os
...
def fetch_and_save_{team_name_underscore}_roster():
    """Fetch and save the {team_name_display} roster from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/{team_name}/roster"
    
    print(f"Fetching roster for {team_name_display} from {{url}}")
    ...
    # Save to CSV
    csv_filename = os.path.join(BASE_DIR, "{team_name}_roster.csv")
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {{csv_filename}}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, "{team_name}_roster.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {{excel_filename}}")
    
    # Print formatted table to terminal
    print(f"\\nRoster for {team_name_display}")
...
if __name__ == "__main__":
    fetch_and_save_{team_name_underscore}_roster()
'''

def generate_team_script(team_slug):
    team_folder = os.path.join(BASE_DIR, team_slug)
    os.makedirs(team_folder, exist_ok=True)
    
    team_name_display = team_slug.replace("-", " ").title()
    team_name_underscore = team_slug.replace("-", "_")
    
    script_content = ROSTER_TEMPLATE.format(
        base_dir=BASE_DIR,
        team_name=team_slug,
    ...
    script_filename = os.path.join(team_folder, f"{team_slug}_roster_scraper.py")
...
print("All roster scripts have been generated successfully!")