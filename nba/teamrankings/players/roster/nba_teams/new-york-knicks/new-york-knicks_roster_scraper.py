...
BASE_DIR = "/Users/kamahl/Sports/scripts/nba/teamrankings/players/roster/nba_teams/new-york-knicks"
os.makedirs(BASE_DIR, exist_ok=True)

def fetch_and_save_new_york_knicks_roster():
    """Fetch and save the New York Knicks roster from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/new-york-knicks/roster"
    
    print(f"Fetching roster for New York Knicks from {url}")
    ...
    # Save to CSV
    csv_filename = os.path.join(BASE_DIR, "new-york-knicks_roster.csv")
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, "new-york-knicks_roster.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table to terminal
    print(f"\nRoster for New York Knicks")
...
if __name__ == "__main__":
    fetch_and_save_new_york_knicks_roster()