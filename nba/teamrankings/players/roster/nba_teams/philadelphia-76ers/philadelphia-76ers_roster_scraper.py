...
BASE_DIR = "/Users/kamahl/Sports/scripts/nba/teamrankings/players/roster/nba_teams/philadelphia-76ers"
os.makedirs(BASE_DIR, exist_ok=True)

def fetch_and_save_philadelphia_76ers_roster():
    """Fetch and save the Philadelphia 76Ers roster from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/philadelphia-76ers/roster"
    
    print(f"Fetching roster for Philadelphia 76Ers from {url}")
    ...
    # Save to CSV
    csv_filename = os.path.join(BASE_DIR, "philadelphia-76ers_roster.csv")
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, "philadelphia-76ers_roster.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table to terminal
    print(f"\nRoster for Philadelphia 76Ers")
...
if __name__ == "__main__":
    fetch_and_save_philadelphia_76ers_roster()