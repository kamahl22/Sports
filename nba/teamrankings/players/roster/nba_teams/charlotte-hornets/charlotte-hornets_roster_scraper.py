...
BASE_DIR = "/Users/kamahl/Sports/scripts/nba/teamrankings/players/roster/nba_teams/charlotte-hornets"
os.makedirs(BASE_DIR, exist_ok=True)

def fetch_and_save_charlotte_hornets_roster():
    """Fetch and save the Charlotte Hornets roster from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/charlotte-hornets/roster"
    
    print(f"Fetching roster for Charlotte Hornets from {url}")
    ...
    # Save to CSV
    csv_filename = os.path.join(BASE_DIR, "charlotte-hornets_roster.csv")
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, "charlotte-hornets_roster.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table to terminal
    print(f"\nRoster for Charlotte Hornets")
...
if __name__ == "__main__":
    fetch_and_save_charlotte_hornets_roster()