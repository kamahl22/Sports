...
BASE_DIR = "/Users/kamahl/Sports/scripts/nba/teamrankings/players/roster/nba_teams/toronto-raptors"
os.makedirs(BASE_DIR, exist_ok=True)

def fetch_and_save_toronto_raptors_roster():
    """Fetch and save the Toronto Raptors roster from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/toronto-raptors/roster"
    
    print(f"Fetching roster for Toronto Raptors from {url}")
    ...
    # Save to CSV
    csv_filename = os.path.join(BASE_DIR, "toronto-raptors_roster.csv")
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, "toronto-raptors_roster.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table to terminal
    print(f"\nRoster for Toronto Raptors")
...
if __name__ == "__main__":
    fetch_and_save_toronto_raptors_roster()