...
BASE_DIR = "/Users/kamahl/Sports/scripts/nba/teamrankings/players/roster/nba_teams/sacramento-kings"
os.makedirs(BASE_DIR, exist_ok=True)

def fetch_and_save_sacramento_kings_roster():
    """Fetch and save the Sacramento Kings roster from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/sacramento-kings/roster"
    
    print(f"Fetching roster for Sacramento Kings from {url}")
    ...
    # Save to CSV
    csv_filename = os.path.join(BASE_DIR, "sacramento-kings_roster.csv")
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, "sacramento-kings_roster.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table to terminal
    print(f"\nRoster for Sacramento Kings")
...
if __name__ == "__main__":
    fetch_and_save_sacramento_kings_roster()