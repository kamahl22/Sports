...
BASE_DIR = "/Users/kamahl/Sports/scripts/nba/teamrankings/players/roster/nba_teams/houston-rockets"
os.makedirs(BASE_DIR, exist_ok=True)

def fetch_and_save_houston_rockets_roster():
    """Fetch and save the Houston Rockets roster from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/houston-rockets/roster"
    
    print(f"Fetching roster for Houston Rockets from {url}")
    ...
    # Save to CSV
    csv_filename = os.path.join(BASE_DIR, "houston-rockets_roster.csv")
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, "houston-rockets_roster.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table to terminal
    print(f"\nRoster for Houston Rockets")
...
if __name__ == "__main__":
    fetch_and_save_houston_rockets_roster()