...
BASE_DIR = "/Users/kamahl/Sports/scripts/nba/teamrankings/players/roster/nba_teams/milwaukee-bucks"
os.makedirs(BASE_DIR, exist_ok=True)

def fetch_and_save_milwaukee_bucks_roster():
    """Fetch and save the Milwaukee Bucks roster from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/milwaukee-bucks/roster"
    
    print(f"Fetching roster for Milwaukee Bucks from {url}")
    ...
    # Save to CSV
    csv_filename = os.path.join(BASE_DIR, "milwaukee-bucks_roster.csv")
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, "milwaukee-bucks_roster.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table to terminal
    print(f"\nRoster for Milwaukee Bucks")
...
if __name__ == "__main__":
    fetch_and_save_milwaukee_bucks_roster()