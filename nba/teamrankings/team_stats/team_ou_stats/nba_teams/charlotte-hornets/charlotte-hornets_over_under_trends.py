...
def fetch_and_save_charlotte_hornets_over_under_trends():
    """Fetch, save, and print the Charlotte Hornets Over/Under trends from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/charlotte-hornets/over-under-trends"
    
    print(f"Fetching Over/Under trends for Charlotte Hornets from {url}")
    ...
    # Save to CSV
    team_name_lower = "charlotte-hornets"
    csv_filename = os.path.join(BASE_DIR, team_name_lower, "charlotte-hornets_over_under_trends.csv")
    os.makedirs(os.path.dirname(csv_filename), exist_ok=True)
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, team_name_lower, "charlotte-hornets_over_under_trends.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table
    print(f"\nOver/Under Trends for Charlotte Hornets")
...
if __name__ == "__main__":
    fetch_and_save_charlotte_hornets_over_under_trends()
