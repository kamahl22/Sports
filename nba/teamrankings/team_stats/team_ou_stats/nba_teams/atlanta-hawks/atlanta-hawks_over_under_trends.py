...
def fetch_and_save_atlanta_hawks_over_under_trends():
    """Fetch, save, and print the Atlanta Hawks Over/Under trends from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/atlanta-hawks/over-under-trends"
    
    print(f"Fetching Over/Under trends for Atlanta Hawks from {url}")
    ...
    # Save to CSV
    team_name_lower = "atlanta-hawks"
    csv_filename = os.path.join(BASE_DIR, team_name_lower, "atlanta-hawks_over_under_trends.csv")
    os.makedirs(os.path.dirname(csv_filename), exist_ok=True)
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, team_name_lower, "atlanta-hawks_over_under_trends.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table
    print(f"\nOver/Under Trends for Atlanta Hawks")
...
if __name__ == "__main__":
    fetch_and_save_atlanta_hawks_over_under_trends()
