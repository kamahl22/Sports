...
def fetch_and_save_los_angeles_clippers_over_under_trends():
    """Fetch, save, and print the Los Angeles Clippers Over/Under trends from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/los-angeles-clippers/over-under-trends"
    
    print(f"Fetching Over/Under trends for Los Angeles Clippers from {url}")
    ...
    # Save to CSV
    team_name_lower = "los-angeles-clippers"
    csv_filename = os.path.join(BASE_DIR, team_name_lower, "los-angeles-clippers_over_under_trends.csv")
    os.makedirs(os.path.dirname(csv_filename), exist_ok=True)
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, team_name_lower, "los-angeles-clippers_over_under_trends.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table
    print(f"\nOver/Under Trends for Los Angeles Clippers")
...
if __name__ == "__main__":
    fetch_and_save_los_angeles_clippers_over_under_trends()
