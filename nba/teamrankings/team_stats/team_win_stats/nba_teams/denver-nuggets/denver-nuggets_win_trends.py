...
def fetch_and_save_denver_nuggets_win_trends():
    """Fetch, save, and print the Denver Nuggets win trends from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/denver-nuggets/win-trends"
    
    print(f"Fetching win trends for Denver Nuggets from {url}")
    ...
    csv_filename = os.path.join(BASE_DIR, team_name_lower, "denver-nuggets_win_trends.csv")
    os.makedirs(os.path.dirname(csv_filename), exist_ok=True)
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, team_name_lower, "denver-nuggets_win_trends.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table
    print(f"\nWin Trends for Denver Nuggets")
...
if __name__ == "__main__":
    fetch_and_save_denver_nuggets_win_trends()
