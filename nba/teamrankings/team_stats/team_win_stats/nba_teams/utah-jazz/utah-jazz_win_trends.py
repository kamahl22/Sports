...
def fetch_and_save_utah_jazz_win_trends():
    """Fetch, save, and print the Utah Jazz win trends from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/utah-jazz/win-trends"
    
    print(f"Fetching win trends for Utah Jazz from {url}")
    ...
    csv_filename = os.path.join(BASE_DIR, team_name_lower, "utah-jazz_win_trends.csv")
    os.makedirs(os.path.dirname(csv_filename), exist_ok=True)
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, team_name_lower, "utah-jazz_win_trends.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table
    print(f"\nWin Trends for Utah Jazz")
...
if __name__ == "__main__":
    fetch_and_save_utah_jazz_win_trends()
