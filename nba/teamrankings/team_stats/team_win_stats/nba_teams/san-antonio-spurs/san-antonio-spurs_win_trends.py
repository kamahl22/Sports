...
def fetch_and_save_san_antonio_spurs_win_trends():
    """Fetch, save, and print the San Antonio Spurs win trends from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/san-antonio-spurs/win-trends"
    
    print(f"Fetching win trends for San Antonio Spurs from {url}")
    ...
    csv_filename = os.path.join(BASE_DIR, team_name_lower, "san-antonio-spurs_win_trends.csv")
    os.makedirs(os.path.dirname(csv_filename), exist_ok=True)
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, team_name_lower, "san-antonio-spurs_win_trends.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table
    print(f"\nWin Trends for San Antonio Spurs")
...
if __name__ == "__main__":
    fetch_and_save_san_antonio_spurs_win_trends()
