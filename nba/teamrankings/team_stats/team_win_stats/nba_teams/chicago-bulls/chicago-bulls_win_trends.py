...
def fetch_and_save_chicago_bulls_win_trends():
    """Fetch, save, and print the Chicago Bulls win trends from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/chicago-bulls/win-trends"
    
    print(f"Fetching win trends for Chicago Bulls from {url}")
    ...
    csv_filename = os.path.join(BASE_DIR, team_name_lower, "chicago-bulls_win_trends.csv")
    os.makedirs(os.path.dirname(csv_filename), exist_ok=True)
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, team_name_lower, "chicago-bulls_win_trends.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table
    print(f"\nWin Trends for Chicago Bulls")
...
if __name__ == "__main__":
    fetch_and_save_chicago_bulls_win_trends()
