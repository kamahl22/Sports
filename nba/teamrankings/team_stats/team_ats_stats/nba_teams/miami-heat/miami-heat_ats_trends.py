...
def fetch_and_save_miami_heat_ats_trends():
    """Fetch, save, and print the Miami Heat ATS trends from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/miami-heat/ats-trends"
    
    print(f"Fetching ATS trends for Miami Heat from {url}")
    ...
    csv_filename = os.path.join(BASE_DIR, team_name_lower, "miami-heat_ats_trends.csv")
    os.makedirs(os.path.dirname(csv_filename), exist_ok=True)
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, team_name_lower, "miami-heat_ats_trends.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table
    print(f"\nATS Trends for Miami Heat")
...
if __name__ == "__main__":
    fetch_and_save_miami_heat_ats_trends()
