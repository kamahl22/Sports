...
def fetch_and_save_orlando_magic_ats_trends():
    """Fetch, save, and print the Orlando Magic ATS trends from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/orlando-magic/ats-trends"
    
    print(f"Fetching ATS trends for Orlando Magic from {url}")
    ...
    csv_filename = os.path.join(BASE_DIR, team_name_lower, "orlando-magic_ats_trends.csv")
    os.makedirs(os.path.dirname(csv_filename), exist_ok=True)
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, team_name_lower, "orlando-magic_ats_trends.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table
    print(f"\nATS Trends for Orlando Magic")
...
if __name__ == "__main__":
    fetch_and_save_orlando_magic_ats_trends()
