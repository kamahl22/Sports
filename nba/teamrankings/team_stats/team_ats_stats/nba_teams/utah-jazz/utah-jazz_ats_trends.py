...
def fetch_and_save_utah_jazz_ats_trends():
    """Fetch, save, and print the Utah Jazz ATS trends from TeamRankings."""
    url = "https://www.teamrankings.com/nba/team/utah-jazz/ats-trends"
    
    print(f"Fetching ATS trends for Utah Jazz from {url}")
    ...
    # Save to CSV
    team_name_lower = "utah-jazz"
    csv_filename = os.path.join(BASE_DIR, team_name_lower, "utah-jazz_ats_trends.csv")
    os.makedirs(os.path.dirname(csv_filename), exist_ok=True)
    df.to_csv(csv_filename, index=False)
    print(f"CSV file saved: {csv_filename}")
    
    # Save to Excel
    excel_filename = os.path.join(BASE_DIR, team_name_lower, "utah-jazz_ats_trends.xlsx")
    df.to_excel(excel_filename, index=False)
    print(f"Excel file saved: {excel_filename}")
    
    # Print formatted table
    print(f"\nATS Trends for Utah Jazz")
...
if __name__ == "__main__":
    fetch_and_save_utah_jazz_ats_trends()
