...
def fetch_and_save_{team_name_underscore}_stats():
    ...
    for category, stats_list in all_stats.items():
        ...
        col_widths = [max(len(str(item)) for item in [stat[0] for stat in stats_list] + ["Trend"]),
                      max(len(str(item)) for item in [stat[1] for stat in stats_list] + ["ATS Record"]),
                      max(len(str(item)) for item in [stat[2] for stat in stats_list] + ["Cover %"]),
                      max(len(str(item)) for item in [stat[3] for stat in stats_list] + ["Mov"])],
                      max(len(str(item)) for item in [stat[3] for stat in stats_list] + ["ATS +/-"])]
