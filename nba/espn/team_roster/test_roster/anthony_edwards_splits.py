...
def fetch_and_save_data(player_id, player_name):
    ...
    all_rows = []
    for table in tables:
        all_rows.extend(table.find_all("tr")[1:])  # Skip header row
    ...
    # Save original CSV in the specified folder
    base_directory = "/Users/kamahl/Sports/scripts_espn/nba/team_roster/nba_team_rosters/nba_teams"
    player_folder = os.path.join(base_directory, "timberwolves", "anthony_edwards")
    try:
        os.makedirs(player_folder, exist_ok=True)
    except PermissionError:
        print(f"Permission denied to create folder: {player_folder}")
        player_folder = os.path.join(os.path.expanduser("~"), "Desktop", "nba_data", "timberwolves", "anthony_edwards")
        os.makedirs(player_folder, exist_ok=True)
    ...
    print(f"Original CSV file saved: {csv_filename}")
    return expected_headers, player_data

def print_excel_style(headers, data):
...
def save_additional_csv(headers, data):
    if not headers or not data:
        print("No headers or data to save as additional CSV.")
        return
    
    # Get the directory where the script is located
    script_dir = os.path.dirname(os.path.abspath(__file__))
    additional_csv_filename = os.path.join(script_dir, f"{player_name.lower().replace(' ', '_')}_splits_local.csv")
    ...
    print(f"Additional CSV file saved in script directory: {additional_csv_filename}")

def main():
    headers, data = fetch_and_save_data(player_id, player_name)
    print(f"Headers: {headers}")
    print(f"Data: {data}")
    print_excel_style(headers, data)
    save_additional_csv(headers, data)  # Save additional CSV in script directory
